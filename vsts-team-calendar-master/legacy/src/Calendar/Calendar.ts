...
export class Calendar extends Controls.Control<CalendarOptions> {
    ...
    public renderEvent(event: Calendar_Contracts.CalendarEvent, eventType: string) {
        ...
        // ← À partir d’ici, injecte le traitement demi-journée :
        if ((<any>event).halfDay === "AM") {
            // matin 9h–12h
            const s = new Date(calEvent.start);
            s.setHours(9,0,0,0);
            calEvent.start = s.toISOString();
            const e = new Date(s);
            e.setHours(12,0,0,0);
            calEvent.end = e.toISOString();

            calEvent.title = "[AM] " + calEvent.title;
            calEvent.color = "#FFCC80";
            calEvent.backgroundColor = calEvent.color;
            calEvent.borderColor = calEvent.color;
        }
        else if ((<any>event).halfDay === "PM") {
            // après-midi 14h–18h
            const s = new Date(calEvent.start);
            s.setHours(14,0,0,0);
            calEvent.start = s.toISOString();
            ...
            calEvent.title = "[PM] " + calEvent.title;
            calEvent.color = "#90CAF9";
            calEvent.backgroundColor = calEvent.color;
            calEvent.borderColor = calEvent.color;
        }
...
this._element.fullCalendar("renderEvent", calEvent, false);
        ...
        // Adapter l'affichage si c'est une demi-journée
        if (event.halfDay === "AM") {
            calEvent.title = "[AM] " + calEvent.title;
            calEvent.color = "#FFCC80"; // Couleur matin
        } else if (event.halfDay === "PM") {
            calEvent.title = "[PM] " + calEvent.title;
            calEvent.color = "#90CAF9"; // Couleur après-midi
        }
    this._element.fullCalendar("renderEvent", calEvent, false);
    }
