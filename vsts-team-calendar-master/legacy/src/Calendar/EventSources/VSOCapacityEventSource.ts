...
export class VSOCapacityEventSource implements Calendar_Contracts.IEventSource {
    ...
    public getCategories(query: Calendar_Contracts.IEventQuery,isHalfDay?: boolean,halfDayType?: "AM" | "PM"): PromiseLike<Calendar_Contracts.IEventCategory[]> {
        if (this._events) {
    ...
    public addEvent(event: Calendar_Contracts.CalendarEvent, isHalfDay?: boolean, halfDayType?: "AM" | "PM"): PromiseLike<Calendar_Contracts.CalendarEvent> {
       // const dayOffStart = new Date(event.startDate);
       // const dayOffEnd = new Date(event.endDate);
       let dayOffStart = new Date(event.startDate);
       let dayOffEnd   = new Date(event.endDate);
        if (isHalfDay) {
            if (halfDayType === "AM") {
                dayOffStart.setHours(9,  0, 0, 0);
                let end = new Date(dayOffStart);
                end.setHours(12, 0, 0, 0);
            } else {
                dayOffStart.setHours(14, 0, 0, 0);
                let end = new Date(dayOffStart);
    ...
    public updateEvent(
        ...
        let dayOffStart = new Date(newEvent.startDate);
        let dayOffEnd   = new Date(newEvent.endDate);
        if (isHalfDay) {
            if (halfDayType === "AM") {
                dayOffStart.setHours(9, 0, 0, 0);
                dayOffEnd = new Date(dayOffStart);
                dayOffEnd.setHours(12, 0, 0, 0);
            } else {
                dayOffStart.setHours(14, 0, 0, 0);
                dayOffEnd = new Date(dayOffStart);
                dayOffEnd.setHours(18, 0, 0, 0);
            }
        }
