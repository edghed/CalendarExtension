...
export class FreeFormEventsSource implements Calendar_Contracts.IEventSource {
                ...
                for (const event of events) {
                    ...
                    if (Date.parse(event.startDate) && Date.parse(event.endDate)) {
                        ...
                        if (start.getHours() !== 0) {
                            ...
                            this.updateEvent(null, event, false, undefined);
    ...
    public updateEvent(
        ...
        newEvent: Calendar_Contracts.CalendarEvent,isHalfDay?: boolean,               
    halfDayType?: "AM" | "PM"
        ...
        return VSS.getService<Services_ExtensionData.ExtensionDataService>(
            ...
            newEvent.category.id = `${this.id}.${newEvent.category.title}`;
                            // Gérer la demi-journée si demandé
                if (isHalfDay) {
                    const start = new Date(newEvent.startDate);
                    if (halfDayType === "AM") {
                        start.setHours(9, 0, 0, 0);
                        newEvent.startDate = start.toISOString();
                        const end = new Date(start);
                        end.setHours(12, 0, 0, 0);
                        newEvent.endDate = end.toISOString();
                    } else if (halfDayType === "PM") {
                        start.setHours(14, 0, 0, 0);
                        newEvent.startDate = start.toISOString();
                        const end = new Date(start);
